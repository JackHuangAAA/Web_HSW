...
public class UsbFingerController {
    ...
    private BroadcastReceiver mPermissionReceiver = new UsbFingerController.PermissionReceiver(new IPermissionListener() {
        public void onPermissionDenied(UsbDevice d) {
            UsbFingerController.this.l("Permission denied on " + d.getDeviceId());
        }
    });
    public static final String TAG = "USBController";

    public UsbFingerController(Context parentActivity, IUsbConnState connectionHandler, int vid, int pid) {
    ...
    public void init() {
        this.enumerate(new UsbFingerController.IPermissionListener() {
            public void onPermissionDenied(UsbDevice d) {
                UsbManager usbman = (UsbManager)UsbFingerController.this.mApplicationContext.getSystemService(Context.USB_SERVICE);
                PendingIntent pi = PendingIntent.getBroadcast(UsbFingerController.this.mApplicationContext,  Toast.LENGTH_LONG, new Intent("ch.serverbox.android.USB"), 0);
                UsbFingerController.this.mApplicationContext.registerReceiver(UsbFingerController.this.mPermissionReceiver, new IntentFilter("ch.serverbox.android.USB"));
    ...
    private void enumerate(UsbFingerController.IPermissionListener listener) {
        ...
        while(deviter.hasNext()) {
            ...
            Toast.makeText(this.mApplicationContext, "Found device: " + String.format("%04X:%04X", d.getVendorId(), d.getProductId()),  Toast.LENGTH_LONG).show();
            if (d.getVendorId() == this.VID && d.getProductId() == this.PID) {
                bFound = true;
                this.l("Device under: " + d.getDeviceName());
                if (this.mUsbManager.hasPermission(d)) {
                    Toast.makeText(this.mApplicationContext, "enumerate, GetConnInerface start",  Toast.LENGTH_LONG).show();
                    this.GetConnInerface(d);
                    return;
                }

                Toast.makeText(this.mApplicationContext, "enumerate, hasPermission return false",  Toast.LENGTH_LONG).show();
        ...
        if (!bFound) {
            Toast.makeText(this.mApplicationContext, "no more devices found", Toast.LENGTH_LONG).show();
    ...
    private class PermissionReceiver extends BroadcastReceiver {
        private final UsbFingerController.IPermissionListener mPermissionListener;

        public PermissionReceiver(UsbFingerController.IPermissionListener permissionListener) {
            this.mPermissionListener = permissionListener;
        }

        public void onReceive(Context context, Intent intent) {
            UsbFingerController.this.mApplicationContext.unregisterReceiver(this);
            if (intent.getAction().equals("ch.serverbox.android.USB")) {
                if (!intent.getBooleanExtra("permission", false)) {
                    ...
                    UsbFingerController.this.mConnectionHandler.onUsbPermissionDenied();
                } else {
                    UsbFingerController.this.l("Permission granted");
                    UsbDevice dev = (UsbDevice)intent.getParcelableExtra("device");
                    if (dev != null) {
                        if (dev.getVendorId() == UsbFingerController.this.VID && dev.getProductId() == UsbFingerController.this.PID) {
                            UsbFingerController.this.GetConnInerface(dev);
                        }
                    } else {
                        UsbFingerController.this.mConnectionHandler.onDeviceNotFound();
                    }
                }
            }
