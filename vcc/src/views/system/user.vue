  ...
  <div>
    ...
    <div class="table">
      <div class="header">
        <Form inline>
          <FormItem>
            账号:<Input v-model="search.userCode"
                   placeholder="账号"
                   style="width: 180px;"></Input>
            用户名称:<Input v-model="search.userName"
                   placeholder="用户名称"
                   style="width: 180px;"></Input>
            <Button type="primary"
                    icon="search"
                    @click="loadOperators('query')">查询</Button>
            <Button type="primary"
                    icon="plus"
                    @click="showAddUserWin">新增</Button>
          </FormItem>
        </Form>
      </div>
      <div class="content">
      ...
      <Form ref="frmEdit"
        ...
        <FormItem label="移动电话:"
          ...
          <Input v-model="formUser.phone"
                 placeholder="请输入移动电话号"
                 :maxlength="11"
                 style="width: 260px;"></Input>
        </FormItem>
        <FormItem label="角色"
                  prop="roles">
          <Select v-model="formUser.roles"
                  style="width:260px">
            <Option v-for="item in roleDatas"
                    :value="item._id"
                    :key="item.id">{{ item.name }}</Option>
          </Select>
        </FormItem>
        <FormItem label="备注">
          <Input v-model="formUser.remark"
                 type="textarea"
        ...
        <div align="right">
          <FormItem style="margin-top: 15px;">
            ...
            <Button type="primary"
                    @click="addOperator('frmEdit')">保存</Button>
          </FormItem>
        </div>
      </Form>
    </Modal>
  ...
  methods: {
    ...
    removeUser: function(row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.$api
            .post("/user/removeUserById", { id: row._id })
            .then(response => {
              this.loadOperators();
