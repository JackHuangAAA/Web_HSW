<template>
  <div class="page">
          ...
          <div class="login">
            ...
            <Form ref="loginFrm"
              ...
              <FormItem>
                ...
                <div class="errortip"
                     v-if="userErrShow"><span>{{usererror}}</span></div>
              </FormItem>
              <FormItem>
                <!--prop="password"-->
                <Input class="login-input"
                       ...
                       placeholder="密码"
                       @on-change="pwdChange"
                       @on-enter="login(loginFrm)">
                <Icon type="ios-lock-outline"
                      slot="prepend"></Icon>
                </Input>
                <div class="errortip"
                     v-if="pwdErrShow"><span>{{pwderror}}</span></div>
                <Checkbox style="padding: 10px 0px 0px 21px;"
                          v-model="rember"
                          @on-change="save()">记住密码</Checkbox>
              </FormItem>
              <!-- <FormItem>
            ...
            <Button type="primary"
                    class="btn"
                    @click="login(loginFrm)"
                    @on-enter="login(loginFrm)">登录</Button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="footer">
      ...
      <hr class="divide" />
      <div class="copyright">
        <span>Copyright © 2018 <a href="http://www.ethinkbank.com"
             target="_blank">浙江银信博荣电子科技股份有限公司</a> All rights reserved.</span>
      </div>
    </div>
  </div>
</template>
