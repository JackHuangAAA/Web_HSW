...
/**
 * @api {GET} /vaccine/queryVaccineNum  查询设备疫苗类型信息
 * @apiGroup Vaccine
 * @apiVersion 1.0.0
 * @apiDescription 查询设备疫苗类型信息
 * @apiParam {String} device 设备id
 * @apiSuccess {Array}  rs  查询疫苗信息
 */
router.get('/queryVaccineNum',
    Libs.router(async (ctx, next) => {
        return await Domain.services.vaccine.queryVaccineNum(ctx.request.query);
    })
);
